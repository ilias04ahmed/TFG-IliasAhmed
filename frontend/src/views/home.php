<div class="relative bg-white overflow-hidden">
            ...
            <main class="mt-10 mx-auto max-w-7xl px-4 sm:mt-12 sm:px-6 md:mt-16 lg:mt-20 lg:px-8 xl:mt-28">
                <div class="sm:text-center lg:text-left">
                    <h1 class="text-4xl tracking-tight font-extrabold text-gray-900 sm:text-5xl md:text-6xl">
                        ...
                        <span
                            class="block text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-teal-500 xl:inline">para
                            Ceuta</span>
                    </h1>
                    <p
    ...
    <!-- Area del Hero -->
    <div
        class="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2 bg-gray-50 flex items-center justify-center overflow-hidden">
        <div
            class="absolute w-96 h-96 bg-blue-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob top-0 left-0">
        </div>
        <div
            class="absolute w-96 h-96 bg-teal-300 rounded-full mix-blend-multiply filter blur-3xl opacity-30 animate-blob animation-delay-2000 bottom-0 right-0">
        </div>

        <div class="relative z-10 text-center transform hover:scale-105 transition duration-700">
            <div class="text-blue-600 filter drop-shadow-2xl animate-float">
            ...
            <div class="mt-8">
                <div
                    class="inline-flex items-center px-4 py-2 rounded-full border border-gray-200 bg-white shadow-sm text-sm font-medium text-gray-500">
...
<div id="avisos-publico" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 hidden">
    ...
    <div id="avisos-publico-list" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
    </div>
</div>
        ...
        async function loadPublicAvisos() {
            try {
                ...
                list.innerHTML = avisos.map(a => {
                    ...
                    const lineaTag = a.linea_codigo
                        ? `<span class="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h8a2 2 0 012 2v10a2 2 0 01-2 2H8a2 2 0 01-2-2V9a2 2 0 012-2z"></path></svg> ${a.linea_codigo}</span>`
                        : '';
                ...
                <div class="rounded-xl border ${cfg.border} ${cfg.bg} p-5 transition transform hover:-translate-y-1 hover:shadow-md">
                    ...
                    <h3 class="font-bold text-gray-900 text-base mb-1">${escapeHtml(a.titulo)}</h3>
                    <p class="text-sm text-gray-600 leading-relaxed">${escapeHtml(a.mensaje)}</p>
                    <p class="text-xs text-gray-400 mt-3">${fecha}</p>
                </div>`;
                }).join('');
            } catch (e) {
                console.log('No se pudieron cargar avisos públicos:', e);
            }
        }
